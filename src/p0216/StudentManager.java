...
public class StudentManager extends StudentDBIO {
    ...
    private static final Pattern SNO_PATTERN = Pattern.compile("^\\d{10}$");//학번 정규 표현식 패턴: 10자리 숫자로 이루어진 문자열
    private static final Pattern NAME_PATTERN = Pattern.compile("^[a-zA-Z가-힣]+$");// 이름 정규표현식 패턴 : 대소문자 A-z 또는 한글로만 이루어진 문자열
    private final Map<Integer, Runnable> menuChoice = new HashMap<>(); //Runnable(동작메서드(run())메서드를 가진 인터페이스)를 Map의 값으로 저장
    ...
    private void mainMenu() {
        menuChoice.put(1, ()-> this.inputStudent()); //(람다)맵의 값으로 Runnable의 run메서드 구현, 해당 키값으로 Runnable호출하여 run메서드를 실행할 수 있다.
    ...
    public void run() {
        while (true) {
            printMenu();//메뉴 목록 메서드 호출
            String input = scanner.nextLine().trim(); //trim() : 양 끝 공백제거

            int choice;

            try {
                choice = Integer.parseInt(input); // string -> int 파싱
                /* str입력받고 int로 파싱할거면 int로 입력받으면 안되나요? 예외처리가 까다로워짐(Pattern은 문자열에 대한 정규표현식을 제공 ) */
            } catch (NumberFormatException e) { //잘못된 입력 잡아내기
                System.out.println("잘못된 입력입니다. 숫자를 입력하세요.");
                continue;
            }
            Runnable action = menuChoice.get(choice); // 입력값 대응 : Runnable 객체 action에 menuchoice Map의 입력받은 키값(choice)에 해당하는 Runnable 객체를 통해 해당하는 run메서드를 실행한다.
            if (action != null) {
                action.run();
            } else {
    ...
    // 숫자 검증 : 입력받은 문자열을 정수로 변환하고 범위 설정, 검증 후 오류 시 안내
        ...
        if (existingStudent == null) { //존재하지 않으면
            ...
            if ("1".equals(option)) {
                ...
                Student student = new Student.StudentBuilder()
                        ...
                        .addSubject("science", science)
                        .build();
                studentDAO.updateStudent(student); //db에 저장된 학생정보 업데이트
                updateInMemoryStudent(student); // 메모리내의 학생 정보를 갱신(기존 학생 데이터 삭제 후 add)
    ...
    //어플리케이션을 종료: 0은 종료, 상수나 범위로 정의하면 에러상태를 나타내는 코드로 사용할 수있다.
